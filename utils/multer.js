...
// UPLOAD MEDIA
function createDestination(destination = 'public/assets', storage) {
  const mediaStorage = {
 ...
 * @destination must be the String (is path of static files). By Default is 'public/assets'
...
exports.editMedia = (params) => async (req, res, next) => {
  ...
  async function editor({ file, width, height, fileName }) {
    await sharp(file)
      ...
      .toFormat('webp')
      .webp({ quality: 90 })
      .toFile(`public/assets/${fileName}`);
  }

  async function writeOriginal({ file, fileName }) {
    await sharp(file).toFormat('webp').webp({ quality: 90 }).toFile(`public/assets/${fileName}`);
  }

  function generateFileName(index) {
    const mainStr = `user-${req.user.id}-${Date.now()}`;
